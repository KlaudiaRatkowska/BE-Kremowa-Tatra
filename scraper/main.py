...
class ProductsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        file_exists = False
	@@ -21,8 +24,8 @@ def parse(self, response):
            pass
        ...
        with open(self.output_filename, 'a', newline='', encoding='utf-8') as file:
            ...
            if not file_exists:
                writer.writeheader()
	@@ -31,7 +34,9 @@ def parse(self, response):
                item = {
	...
	@@ -44,38 +49,52 @@ def parse(self, response):
...
if __name__ == "__main__":
    ...
    for task in tasks:
        ...
        processes.append(p)
	@@ -85,4 +104,4 @@ def run_spider(file_path, output_file_name):
        p.join()
